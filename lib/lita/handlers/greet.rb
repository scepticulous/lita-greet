module Lita
  module Handlers
    class Greet < Handler
      ...
      route(/^(((ahoy|he+l{2,}+o+|he+y+|hi+)+( there)*)|go{2,}d (morning|afternoon|evening|night|nite)|greetings|howdy|moin|sup|yo)+[!1?]*((\s)+Regexp.new(robot.config.robot.name))*[!1?]*$/i, :say_hello)
      ...
      def say_hello(response)
        ...
        response.reply "#{bot_response} #{(response.user.name || response.user.mention_name)}"
      end
